  ...
  async run(message, member) {
    ...
    collector.on('collect', async (b) => {
      ...
      if (b.customId == `confirmClose`) {
        ...
        if(this.client.config.general.rename_close == true) {

          message.channel.setName(this.client.config.channels.closed_name.toLowerCase() + this.client.utils.ticketUsername(member.user))
