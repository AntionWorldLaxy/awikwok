...
module.exports = class GetProduct extends Command {
  ...
  async run(message, args) {
    ...
    if(selectedProduct.roles.length > 0 && !this.client.utils.hasRole(this.client, message, selectedProduct.roles)) 

      return message.channel.send({ embeds: [this.client.embedBuilder(this.client, message.author, this.client.embeds.title, this.client.language.products.roles.replace("<role>", selectedProduct.roles.join(", ").trim()), this.client.embeds.error_color)] });
  ...
  async slashRun(interaction, args) {
    ...
    if(selectedProduct.roles.length > 0 && !this.client.utils.hasRole(this.client, interaction, selectedProduct.roles)) 

      return interaction.reply({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.products.roles.replace("<role>", selectedProduct.roles.join(", ").trim()), this.client.embeds.error_color)], ephemeral: this.client.cmdConfig.getproduct.ephemeral });
